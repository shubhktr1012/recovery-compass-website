...
export function normalizeProgramValues(values: string[]) {
  return Array.from(
    new Set(
      values.map((value) => PROGRAM_VALUE_ALIAS_BY_NAME[value] ?? value)
    )
  );
}

export function normalizeQuestionnaireProgramValues<T extends Record<string, unknown>>(questionnaireData: T): T {
  ...
  return {
    ...
    programs: normalizeProgramValues(
      questionnaireData.programs.filter((program): program is string => typeof program === "string")
    ),
  };
}
