...
const PANEL_SHELL =
  "overflow-hidden rounded-[2rem] border border-white/10 bg-[radial-gradient(circle_at_0%_0%,rgba(167,139,250,0.14),transparent_32%),radial-gradient(circle_at_95%_8%,rgba(139,211,255,0.1),transparent_28%),rgba(255,255,255,0.045)]";
...
function InnerCard({
  ...
  return (
    <Card
      className={cn(
        INNER_CARD,
        accent === "violet" && "border-violet-200/15 bg-violet-300/10",
        accent === "teal" && "border-teal-200/20 bg-teal-300/[0.08]",
        className
      )}
    >
      <CardHeader className="border-b border-white/10 px-4 py-3">
        <CardTitle
          className={cn(
            ...
            accent === "violet" && "text-violet-100/70",
            accent === "teal" && "text-teal-100/75",
  ...
  return (
    <section className={PANEL_SHELL}>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <div className="flex items-center gap-2 text-violet-100">
              ...
              <h2 className="text-lg font-semibold text-white">AI user summary</h2>
            </div>
      ...
      <div className="space-y-4 p-5">
        {isLoading ? (
          <div className="flex items-center gap-3 rounded-[1.35rem] border border-white/10 bg-black/20 px-4 py-6 text-sm text-white/60">
            <Loader2 className="size-4 animate-spin text-violet-100" />
            Loading AI summary…
          </div>
        ) : null}

        {error ? (
          <Alert className="border-rose-200/15 bg-rose-300/10 text-rose-50 shadow-none">
            <AlertTitle className="text-rose-100">Summary failed</AlertTitle>
            <AlertDescription className="text-rose-50/90">{error}</AlertDescription>
          </Alert>
        ) : null}

        {summary ? (
          <>
            <InnerCard accent="violet" title="Headline">
              <p className="text-base font-medium leading-7 text-white">
                {summary.insights.headline}
              </p>
              <div className="mt-3 flex flex-wrap gap-2">
                <Badge className="border-white/10 bg-white/10 text-white hover:bg-white/10" variant="outline">
                  {payload?.model}
                </Badge>
                <Badge className="border-white/10 bg-white/10 text-white hover:bg-white/10" variant="outline">
            ...
            <InnerCard accent="teal" title="Sales & outreach">
            ...
            {summary.insights.risks.length > 0 ? (
              <div className="space-y-2">
                {summary.insights.risks.map((risk) => (
                  <Alert
                    className="border-amber-200/15 bg-amber-300/10 text-amber-50 shadow-none"
                    key={risk}
                  >
            ...
            <InnerCard accent="teal" title="Next best action">
            ...
            {payload?.context ? (
              <Card className={INNER_CARD}>
                <button
                  className="flex w-full items-center justify-between px-4 py-3 text-left text-sm font-medium text-white/70 transition hover:bg-white/[0.04]"
          ...
          <Alert className="border-amber-200/15 bg-amber-300/10 text-amber-50 shadow-none">
            <AlertTitle className="text-amber-100">Summary format outdated</AlertTitle>
